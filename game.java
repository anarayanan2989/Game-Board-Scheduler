...
public final class game
{
	public static void main(String args[]) throws Exception
	{
		...
		int[] [] task = new int[token] [5];
		int[] [] executable_task = new int[token] [5];
		int[] [] idle_task = new int[token-processors] [5];
			...
			for(int time = 0;time <= max_deadline;time++)
			{
				...
				if(time==min_ScheduleTime)
				{
					...
					if((count < processors)||(count == processors)){
						...
						for(int row = 0;row<idle_task.length; row++)
						{
							...
							for(int trow = 0;trow < task.length;trow++){
									if(idle_task[row][5] == executable_task[trow][5])
										idle_task[row][1] = executable_task[trow][1];
									else
										continue;
							}
						}
						...
						EDF_Tester(executable_task,idle_task);
...
public static void EDF_Tester(int [][] task1, int[][] task2)
	{
		...
		for(int irow = 0; irow<task2.length; irow++)
		{
			...
			{
				if(task1[row][1] == 0)
					System.out.println("Job-" +task1[row][4]+ " is completed");
